...
class SetTransformerDataset(Dataset):
    def __init__(self, config, channel_groups, hdf5_paths=[], split="pretrain"):
        ...
        self.samples_per_chunk = (
            config["sampling_duration"] * 60 * config["sampling_freq"]
        )
    ...
    def __getitem__(self, idx):
        ...
        modality_to_channels = {
            modality_type: [] for modality_type in self.config["modality_types"]
        }
        ...
        with h5py.File(file_path, "r", rdcc_nbytes=300 * 512 * 8 * 2) as hf:
            for modality_type in self.config["modality_types"]:
                ...
                data = np.zeros(
                    (len(modality_to_channels[modality_type]), self.samples_per_chunk)
                )
                ds_names = modality_to_channels[modality_type]
                for idx, ds_name in enumerate(ds_names):
                    signal = hf[ds_name][
                        chunk_start : chunk_start + self.samples_per_chunk
                    ]
                    data[idx] = signal
...
def collate_fn(batch):
    ...
    for modality_index in range(num_modalities):
        ...
        padded_batch_list[modality_index] = torch.stack(
            padded_batch_list[modality_index]
        )
...
class SleepEventClassificationDataset(Dataset):
    def __init__(self, config, channel_groups, hdf5_paths=[], split="train"):
        self.config = config
        ...
        for dataset_name in dataset:
            label_files += glob.glob(
                os.path.join(labels_path, dataset_name, "**", "*.csv"), recursive=True
            )
        ...
        hdf5_paths = [os.path.join(data_path, path) for path in hdf5_paths]
        study_ids = set(
            [os.path.basename(label_file).split(".")[0] for label_file in label_files]
        )

        hdf5_paths = [f for f in hdf5_paths if os.path.exists(f)]
        hdf5_paths = [
            f for f in hdf5_paths if f.split("/")[-1].split(".")[0] in study_ids
        ]

        hdf5_paths_ids = set(
            [os.path.basename(hdf5_path).split(".")[0] for hdf5_path in hdf5_paths]
        )

        hdf5_paths_new = []
        for dataset_name in dataset:
            hdf5_paths_new += glob.glob(
                os.path.join(config["model_path"], dataset_name, "**", "*.hdf5"),
                recursive=True,
            )

        hdf5_paths_new = [
            item
            for item in hdf5_paths_new
            if os.path.basename(item).split(".")[0] in hdf5_paths_ids
        ]
        ...
        if self.context == -1:
            self.index_map = [
                (path, labels_dict[path.split("/")[-1].split(".")[0]], -1)
                for path in hdf5_paths
            ]
        else:
            self.index_map = []
            loop = tqdm(
                hdf5_paths[:], total=len(hdf5_paths), desc=f"Indexing {split} data"
            )
                ...
                with h5py.File(hdf5_file_path, "r") as file:
                    ...
                    for i in range(0, dataset_length, self.context):
                        self.index_map.append(
                            (hdf5_file_path, labels_dict[file_prefix], i)
                        )

        logger.info(f"Number of files in {split} set: {len(hdf5_paths)}")
        logger.info(
            f"Number of files to be processed in {split} set: {len(self.index_map)}"
        )
        self.total_len = len(self.index_map)
            ...
            for dataset_name in dset_names:
                if dataset_name in self.channel_like:
                    if self.context == -1:
                        ...
                        x_data_in = hf[dataset_name][
                            start_index : start_index + self.context
                        ]
...
class DiagnosisFinetuneFullCOXPHDataset(Dataset):
    def __init__(self, config, channel_groups, hdf5_paths=[], split="train"):
        ...
        hdf5_paths = [
            f
            for f in glob.glob(
                os.path.join(config["model_path"], config["dataset"], "**", "*.hdf5"),
                recursive=True,
            )
        ...
        self.index_map = [
            (path, labels_dict[path.split("/")[-1].split(".")[0]])
            for path in hdf5_paths
        ]
        logger.info(f"Number of files in {split} set: {len(hdf5_paths)}")
        logger.info(
        ...
        try:
            with h5py.File(hdf5_path, "r") as hf:
                ...
                for modality_type in self.config["modality_types"]:
                    ...
                    modality_channels = list(
                        set(self.channel_groups[modality_type]) & set(dset_names)
                    )
                    if len(modality_channels) > max_modality_channels:
                        modality_channels = random.sample(
                            modality_channels, max_modality_channels
                        )
...
def supervised_diagnosis_full_coxph_with_demo_collate_fn(batch):
    ...
    max_seq_len = (
        max_seq_len_list[0]
        if max_seq_len_list[0] is not None
        else max([item.size(1) for item in x_data])
    )
