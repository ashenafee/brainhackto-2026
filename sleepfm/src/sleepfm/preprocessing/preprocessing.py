...
class EDFToHDF5Converter:
    def __init__(
        self, root_dir, target_dir, resample_rate=512, num_threads=1, num_files=-1
    ):
    ...
    def get_files(self):
        ...
        file_paths = glob.glob(
            os.path.join(self.root_dir, "**/*.[eE][dD][fF]"), recursive=True
        )
    ...
    def create_signal_from_events(self, df, total_seconds, event_type=None):
        ...
        for _, row in df.iterrows():
            ...
            event_stop = int(
                row["sec_from_start"] * self.resample_rate
                + row["dur"] * self.resample_rate
            )
            if event_type == "flow":
    ...
    def make_event_dataframe(self, folder, event_type=None):
        ...
        if event_type == "flow":
            ...
            df = pd.read_csv(
                ...
                delimiter=";",
                skiprows=5,
                names=["start-stop", "duration", "event_type"],
            )
        elif event_type == "plm":
            ...
            flow_file = os.path.join(
                self.root_dir, folder, "Classification arousals.txt"
            )
            dfClassification = pd.read_csv(
                flow_file,
                delimiter=";",
                skiprows=5,
                names=["start-stop", "duration", "event_type"],
            ...
            df = pd.read_csv(
                flow_file, delimiter=";", skiprows=7, names=["start", "event_type"]
            )
            ...
            df["sec_from_start"] = df["start"].apply(
                lambda x: (
                    ...
                    - datetime.datetime.combine(
                        datetime.date(1, 1, 1), start_time.time()
                    )
                ).total_seconds()
            )
            df["dur"] = 30
    ...
    def convert_events(self, folder, total_seconds, event_type):
        ...
        event_array = self.create_signal_from_events(
            df=df_events, total_seconds=total_seconds, event_type=event_type
        )
        return event_array
    ...
    def read_edf_old(self, file_path):
        ...
        with pyedflib.EdfReader(file_path) as edf:
            ...
            channel_names = np.array(
                [edf.getLabel(i) for i in range(edf.signals_in_file)]
            )
    ...
    def resample_signals_old(self, signals, sample_rates):
        ...
        standardized_signals = [
            (signal - np.mean(signal)) / np.std(signal) for signal in resampled_signals
        ]
        # add signal names as input and filter the spo2 preprocessing as scaled 0 to 1
        return np.stack(
            standardized_signals
        )  # Using np.stack for proper array dimensions
    ...
    def resample_signals(self, signals, sample_rates):
        ...
        for signal, rate in zip(signals, sample_rates):
            ...
            original_time_points = np.linspace(
                0, duration, num=len(signal), endpoint=False
            )

            # New sample rate and new time points
            new_sample_count = int(duration * self.resample_rate)
            new_time_points = np.linspace(
                0, duration, num=new_sample_count, endpoint=False
            )
    ...
    def save_to_hdf5(
        self, signals, channel_names, annotation_signals, annotation_names, file_path
    ):
    ...
    def get_annotations(self, total_seconds, folder):
        ...
        flow_events = self.convert_events(
            folder=folder, total_seconds=total_seconds, event_type="flow"
        )
        plm_events = self.convert_events(
            folder=folder, total_seconds=total_seconds, event_type="plm"
        )
        arousal_events = self.convert_events(
            folder=folder, total_seconds=total_seconds, event_type="arousal"
        )
        sleep_stages = self.convert_events(
            folder=folder, total_seconds=total_seconds, event_type="stages"
        )
    ...
    def convert_multiprocessing(self, args):
        ...
        for edf_file in tqdm(edf_files, desc="Converting EDF files"):
            ...
            hdf5_file = os.path.join(
                self.target_dir, edf_file.split("/")[-1].replace(replace_str, ".hdf5")
            )
            ...
            try:
                ...
                warnings.warn(
                    f"Warning: Could not process the file {edf_file}. Error: {str(e)}"
                )
                continue
        return [1]
    ...
    def convert_all_multiprocessing(self):
        ...
        with multiprocessing.Pool(self.num_threads) as pool:
            preprocessed_results = [
                y
                for x in pool.imap_unordered(self.convert_multiprocessing, tasks)
                for y in x
            ]
    ...
    def convert_with_annot(self, edf_path, hdf5_path, folder):
        ...
        for scorer in self.scorers:
            ...
            flow_events, plm_events, arousal_events, sleep_stages = (
                self.get_annotations(
                    total_seconds=total_duration_seconds, folder=scorer_folder
                )
            )
    ...
    def plot_results(self, resampled_signals, channel_names):
        ...
        fig, axs = plt.subplots(
            num_signals, 1, figsize=(15, 3 * num_signals), sharex=True
        )
...
def main():
    ...
    parser.add_argument(
        "--target_dir", type=str, required=True, help="Path to save hdf5"
    )
    parser.add_argument(
        "--num_threads",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of files to process. If -1, process all",
    )
    parser.add_argument(
        "--resample_rate",
        type=int,
        default=256,
        help="Target sampling rate for hdf5 file.",
    )
